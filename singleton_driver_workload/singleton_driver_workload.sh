...
# Initial declaration of unreachable assertion - single line json written as per docs.
echo "{\"antithesis_assert\": {\"hit\": false, \"must_hit\": false, \"assert_type\": \"reachability\", \"display_type\": \"Unreachable\", \"message\": \"Storage vaults in inconsistent state\", \"condition\": false, \"id\": \"Storage vaults in inconsistent state\", \"location\": {\"class\": \"\", \"function\": \"\", \"file\": \"/opt/antithesis/test/v1/main/singleton_driver_workload.sh\", \"begin_line\": 56, \"begin_column\": 0}, \"details\": null}}" > "$ANTITHESIS_OUTPUT_DIR/sdk.json"
...
do
    n=$(( $n + (($RANDOM % 4) - 1)*(($RANDOM % $MAX_STEP_SIZE) + 1 )))
    outfile="/tmp/${n}.out"
    ...
    then
        ...
        then
            ...
            # Unreachable assertion here - single line json written as per docs.
            echo "{\"antithesis_assert\": {\"hit\": true, \"must_hit\": false, \"assert_type\": \"reachability\", \"display_type\": \"Unreachable\", \"message\": \"Storage vaults in inconsistent state\", \"condition\": false, \"id\": \"Storage vaults in inconsistent state\", \"location\": {\"class\": \"\", \"function\": \"\", \"file\": \"/opt/antithesis/test/v1/main/singleton_driver_workload.sh\", \"begin_line\": 56, \"begin_column\": 0}, \"details\": null}}" > "$ANTITHESIS_OUTPUT_DIR/sdk.json"
            # 
        elif [[ "${n}" != "${actual}" ]]
        then
